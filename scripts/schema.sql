-- ================================================================
-- API-Football 통합을 위한 DB 스키마 추가
-- ================================================================

-- 1. 팀 ID 매핑 테이블
CREATE TABLE IF NOT EXISTS team_mapping (
  our_team_id VARCHAR(30) PRIMARY KEY,  -- "MEX", "KOR", "BRA" 등
  api_team_id INTEGER NOT NULL UNIQUE,  -- API-Football team ID
  api_team_name VARCHAR(100),           -- API에서의 팀 이름
  api_team_code VARCHAR(10),            -- API team code (BRA, KOR 등)
...
CREATE TABLE IF NOT EXISTS team_statistics (
  ...
  yellow_cards INTEGER DEFAULT 0,       -- 옐로카드 수
  two_yellow_red INTEGER DEFAULT 0,     -- 2장 누적 퇴장
  direct_red INTEGER DEFAULT 0,         -- 직접 레드카드
  fair_play_points INTEGER DEFAULT 0,   -- 페어플레이 점수 (낮을수록 좋음)
...
CREATE TABLE IF NOT EXISTS api_sync_log (
  ...
  sync_type VARCHAR(50) NOT NULL,      -- 'fixtures', 'team_stats', 'full_sync' 등
...
-- 4. match_results 테이블 확인 (이미 있어야 함)
-- 기존 테이블 구조 확인용 주석
/*
...
-- 5. 뷰: 팀별 종합 통계 (match_results + team_statistics 결합)
CREATE OR REPLACE VIEW v_team_full_stats AS
SELECT
  ts.team_id,
  ts.group_key,
  ts.yellow_cards,
...
-- ================================================================
-- 초기 데이터: 48개 팀의 team_statistics 레코드 생성 (카드 0으로 초기화)
-- ================================================================
...
-- ================================================================
-- 스키마 추가 완료
-- 다음 단계: scripts/generateTeamMapping.js 실행하여 team_mapping 데이터 생성
